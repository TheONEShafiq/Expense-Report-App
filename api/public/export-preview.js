...
async function load(){ const actor = els.actor.value.trim() || localStorage.getItem('actorEmail'); const id = els.reportId.value.trim(); if(!actor || !id){ alert('Enter actor email and report ID'); return; } const j = await fetchJSON(`/exports/${id}/preview`, actor); els.cache = j.data; els.meta.textContent = JSON.stringify(j.data.report, null, 2); renderLines(); }
...
els.dlIif.addEventListener('click', async () => { const actor = els.actor.value.trim() || localStorage.getItem('actorEmail'); const id = els.reportId.value.trim(); if(!actor || !id){ alert('Enter actor email and report ID'); return; } const r = await fetch(`/exports/${id}/qbd.iif`, { headers: { 'X-Actor-Email': actor } }); if(!r.ok){ alert(await r.text()); return; } const blob = await r.blob(); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `qbd_${id.slice(0,8)}.iif`; a.click(); });
