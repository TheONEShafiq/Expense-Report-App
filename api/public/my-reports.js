...
async function loadCompanies(){ const j = await fetch('/admin/companies').then(r=>r.json()); els.company.innerHTML = j.data.map(c => `<option value="${c.id}">${c.name}</option>`).join(''); const storedCompany=localStorage.getItem('companyId'); if(storedCompany){ els.company.value = storedCompany; } }
async function loadReports(){
  const actor = els.actor.value.trim() || localStorage.getItem('actorEmail'); if(!actor) return;
  ...
  els.repBody.querySelectorAll('tr').forEach(tr => {
    ...
    if (submitBtn) submitBtn.addEventListener('click', async () => { const actor = els.actor.value.trim() || localStorage.getItem('actorEmail'); await postJSON(`/reports/${id}/submit`, {}, actor); alert('Submitted. Auto-approvals applied.'); await loadReports(); });
    if (reopenBtn) reopenBtn.addEventListener('click', async () => { const actor = els.actor.value.trim() || localStorage.getItem('actorEmail'); await postJSON(`/reports/${id}/reopen`, {}, actor); alert('Reopened to Draft.'); await loadReports(); });
...
els.company.addEventListener('change', () => { localStorage.setItem('companyId', els.company.value); loadReports(); });
els.actor.addEventListener('change', () => { localStorage.setItem('actorEmail', els.actor.value.trim()); loadReports(); });
