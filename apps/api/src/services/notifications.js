...
import { prisma } from "../db/prisma.js"; // singleton Prisma client
...
export async function notifyApproversOnSubmit(reportId) {
  ...
  // find approvers in same company
  const approverIds = await prisma.user_roles.findMany({
    where: { role: { key: "approver" } },
    select: { user_id: true },
  });
