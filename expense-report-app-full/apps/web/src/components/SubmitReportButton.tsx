...
export const SubmitReportButton: React.FC<{ reportId: number; className?: string }> = ({ reportId, className }) => {
  ...
  return (
    <button onClick={onSubmit} disabled={loading} className={`px-4 h-10 rounded-2xl border bg-[#1E242B] border-[#2B323A] text-[#E6E8EB] hover:bg-[#222831] disabled:opacity-60 ${className ?? ""}`}>
